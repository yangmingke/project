...
import com.flypaas.dao.TbRsMailPropMapper;
...
public class MailServiceImpl  implements MailService {
	protected TbRsUserInfo tbRsUserInfo;
	private MimeMessage mimeMessage ;
	@Autowired
	private JavaMailSender mailSender;
	@Autowired
	private TbRsMailPropMapper tbRsMailPropMapper;
	...
	public String getUrl(){
        ...
        try {
			...
			pName.load(iStr);
			url = pName.getProperty("mail_url");
		} catch (IOException e) {
			e.printStackTrace();
		}
		return url;
	...
	private TbRsMailProp getMailByType(String type){
		TbRsMailProp p = tbRsMailPropMapper.getMailByType(type);
		return p ;
	}
	
	public void sendValidateMail(String id,String netId,String code,String time,String email) {
		log.info("开始发送邮件："+email);
		TbRsMailProp p = getMailByType(SysConstant.V_MIAL_TYPE);
		...
		try {
			...
			text = text.replaceAll("vhost",getUrl()).replaceAll("vsid",id).replaceAll("vnetId", netId).replaceAll("vcode", code).replaceAll("vemail", email).replaceAll("vtime", DateUtil.dateToStr(DateUtil.getCurrentDate(), "yyyyMMdd"));
	...
	/*public void sendLogSucMail(String email) {
		log.info("开始发送邮件:"+email);
		TbRsMailProp p = getMailByType(SysConstant.LOGSUC_MIAL_TYPE);
		mimeMessage = getMimeMessage();
		try {
			MimeMessageHelper messageHelper = new MimeMessageHelper(mimeMessage, true, SysConstant.CHARSET);
			messageHelper.setFrom(p.getFrm(), p.getFromnickname());
