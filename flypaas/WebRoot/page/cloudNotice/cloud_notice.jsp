...
$(function(){
	...
	function del(id){
		 location.href = '<%=path%>/app/delCloudNotice?id=' + id;
	}
</script>
</head>
	...
	<div class="main">
		...
		<div class="search_box">
			<div class="search app_search">
				<s:form namespace="/app" theme="simple"  action="cloudNotice" method="post" name="queryForm" id="queryForm"  >
			      	<input type="text" name="text" placeholder="应用名称/ID" value="<s:property value="#parameters.text"/>" />
			      	<input type="submit" value="" class="search" />
			      </s:form>
			</div>
			<div class="search_link"><a href="<%=path%>/app/addCnPage" class="voice">添加语音</a></div>
		</div>
		<!--搜索search_box eof-->
	    <!--表格列表 table_box bof-->
		<div class="table_box">
	     ...
	     <s:if test="page.list!=null&&page.list.size()>0">
		     <s:iterator value="page.list" var="r">
			      ...
			      <td>
			     		<a href="javascript:void(0);" onclick="play_fun('<%=path%>/file/view?fileName=<s:property value="#r.file_path" />');">播放</a>
				      	<a href="javascript:void(0);" onclick="popupBox('删除短信模板','是否删除短信模板','del(\'<u:des3 value="${r.id}"/>\')',1)">删除</a>
			      </td>
			      </tr>
		      </s:iterator>
	      </s:if>
	      <s:else>
	      	<tr>
	      	<td>暂无数据! &nbsp;&nbsp;<a href="<%=path%>/app/addCnPage" style="color:#4981bf">去上传语音</a></td>
	      	<td>&nbsp;</td>
	      	<td>&nbsp;</td>
	      	<td>&nbsp;</td>
	      	<td>&nbsp;</td>
