...
public class RestClient {
	...
	static{
		displayNum = SysConfig.getInstance().getProperty("displaynum");
	}
	 ...
	 * @return
	 */
	public static String createClient (String accountSid,String authToken,String appId){
		String result = null ;
		try {
			result=InstantiationRestAPI().createClient(accountSid, authToken, appId);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("--------------------------生成client number 失败------------------------");
			logger.error(e.getMessage());
	 ...
	 * @return
	 */
	public static String bandClients(String accountSid, String authToken,String appId,
			String clientNumber,String mobile){
		String result = null ;
		try {
			String cType = "1" ;
			result=InstantiationRestAPI().bandClients(accountSid, authToken, appId, clientNumber, mobile,cType);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("--------------------------回拨失败------------------------");
			logger.error(e.getMessage());
	...
	public static String cloudNotify(String accountSid, String authToken, String appId,
			String to, String type, String content){
		String result = null ;
		try {
			result=InstantiationRestAPI().cloudNotify(accountSid, authToken, appId, to, type, content);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("--------------------------回拨失败------------------------");
			logger.error(e.getMessage());
