...
public class StrUtil {
	...
	public static String replaceNewsImageStr(String target){
		String regex = SysConstant.NEWS_IMG_PREFIX;
		Pattern pattern = Pattern.compile(regex);
		Matcher m = pattern.matcher(target);
		while(m.find()){
				String all=m.group(0);
				String url=m.group(2);
				target = target.replace(all, SysConstant.UCP_NEWS_IMG_PREFIX+Des3Utils.encodeDes3(url)+"\"");
		}
		return target;
	}
	...
	public static String replaceNewsDownImageStr(String target){
		String regex = SysConstant.NEWS_DOWNLOAD_IMG_PREFIX;
		Pattern pattern = Pattern.compile(regex);
		Matcher m = pattern.matcher(target);
		while(m.find()){
			String all=m.group(0);
			String url=m.group(2);
			target = target.replace(all, SysConstant.UCP_NEWS_DOWNIMG_PREFIX+Des3Utils.encodeDes3(url)+"\"");
		}
		return target;
	}
	//加密sid token
	...
	public static String getCookiValue(HttpServletRequest request,String cookieName) {
