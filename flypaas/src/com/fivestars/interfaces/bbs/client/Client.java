...
public class Client extends PHPFunctions{
	...
	static {
		String name ="config/uc_center.properties";
		if("production".equals(System.getProperty("spring.profiles.active"))){
			name = "config/uc_center_production.properties";
		}else if("test".equals(System.getProperty("spring.profiles.active"))){
			name = "config/uc_center_test.properties";
		}
		InputStream in = Client.class.getClassLoader().getResourceAsStream(name);
...
//		}
	...
	public String uc_api_post(String $module, String $action, Map<String,Object> $arg ) {
		...
		for (String $k : $arg.keySet()) {
			...
			if(null != $v && $v.getClass().isAssignableFrom(Map.class)) {
				String $s2 = "";
				String $sep2 = "";
				//foreach($v as $k2 => $v2) {
