...
public class HttpUtils {
	...
	public static String postForm(String url, Map<String, Object> data) {
		...
		try {
			...
			for (Map.Entry<String, Object> d : data.entrySet()) {
				if (d.getValue() != null) {
					formParams.add(new BasicNameValuePair(d.getKey(), d
							.getValue().toString()));
				}
			}
			...
			logger.debug("【发送post请求（表单）】成功：url=" + url + ", data=" + data
					+ ", result=" + result);
		} catch (Throwable e) {
			logger.error("【发送post请求（表单）】失败：url=" + url + ", data=" + data
					+ ", result=" + result, e);
		}
		return result;
	}
	...
	public static String postJson(String url, Object data) {
		...
		String result = null;
		Request request = Request.Post(url);
		request.setHeader("Accept", contentType.getMimeType());
		request.setHeader("Content-Type", contentType.toString());
		request.bodyString(dataStr, contentType);
		try {
			result = request.execute().returnContent().asString();
			logger.debug("【发送post请求（json字符串）】成功：url=" + url + ", data=" + data
					+ ", result=" + result);
		} catch (Throwable e) {
			logger.error("【发送post请求（json字符串）】失败：url=" + url + ", data=" + data
					+ ", result=" + result, e);
		}
		return result;
	}
	...
	public static void main(String[] args) {
		...
		postJson("http://www.ucpaas.com/maap/voice/code", data);
	}
