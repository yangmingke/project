...
public class ConKeyService {
	...
	public synchronized Map<String, String> getNextNbr(){
		...
		DBCommService.getInstance().update(SqlCode.UPDATE_NUM,px,true);
		return nextMap;
	}
	...
	public synchronized void setNumbers(){
		...
		if (queue.size()<=0) {
			if (getCurNum()==0) {
				try {
					long curNum=DBCommService.getInstance().queryForLong(SqlCode.NEXT_VALUE_NUM,null,false);//RedisService.getInstance().get(Consts.KEY_CURNUM);
					if (curNum!=0) {
						start=curNum+1;
					}
					/*if (curNum!=null) {
