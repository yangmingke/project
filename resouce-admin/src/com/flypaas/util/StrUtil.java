...
public class StrUtil {
	...
	public static void writeMsg(HttpServletResponse response, String msg,String url){
		...
		sb.append("<link rel='stylesheet' type='text/css' href='/css/form.css' />");
		sb.append("</head>");
	...
	/**
	 * 获取config_XXXX.properties的值
	 * @param key
	 * @return
	 * @throws IOException 
	 */
	public static String getConfigVal(String key) throws IOException{
        ...
        InputStream iStr = StrUtil.class.getResourceAsStream("/config_"+spring_profiles_active+".properties");
        pName.load(iStr);
        String value = pName.getProperty(key);
		return value;
	}
	/**
	 * 字符串数字化作数字比较大小，首参数大于返回true
	 * @param str
	 * @param compareStr
	 * @return
	 */
	public static boolean IsBigger(String str,String compareStr){
		return Double.valueOf(str) > Double.valueOf(compareStr);
	}
	/**
	 * 比较ip地址大小，首首参数大于返回true
	 * @param ipAddress
	 * @return
	 */
	public static boolean firstIpAddressIsLess(String firstIp,String secondIp){
		String[] firstNumArray= firstIp.split("\\.");
		String[] secondNumArray= secondIp.split("\\.");
		for(int i = 0; i < firstNumArray.length; i++){
			if(IsBigger(firstNumArray[i],secondNumArray[i])){
				return false;
			}else if(IsBigger(secondNumArray[i],firstNumArray[i])){
				return true;
			}
		}
		return false;
