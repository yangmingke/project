...
//查看
function view(sid){
	location.href=ctx +"/agentAccount/view?sid=" + sid;
}
//初始化弹层(冻结,解冻,注销) 
...
function updateEnableFlag(boxId){
	...
	$.ajax({
		type: "post",
		url: ctx + "/agentAccount/updateEnableFlag",
		data:{
			mngType : mngType,
...
function updateBalance(boxId){
	...
	if(window.confirm("您确定需要 [ "+mngType+" "+changeValue+" ] 吗?")){
		...
		$.ajax({
			type: "post",
			url: ctx + "/agentAccount/updateBalance",
			data:{
				mngType : mngType,
				sid : sid,
...
function saveCreditBalance(sid,acct_id){
	...
	if(/^(([0-9]|([1-9][0-9]{0,9}))((\.[0-9]{1,4})?))$/.test(credit_balance)){
			if(window.confirm("您确定需要设置信用额度为 "+credit_balance+" 元 吗")){
				$.post(ctx + "/agentAccount/saveCreditBalance",{
					'sid':sid,
					'acct_id':acct_id,
