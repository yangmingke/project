...
201	上传
 -->
<u:authority menuId="199">
	<s:set var="menuId_199" value="true"/>
</u:authority>
<u:authority menuId="200">
	<s:set var="menuId_200" value="true"/>
</u:authority>
<u:authority menuId="201">
	<s:set var="menuId_201" value="true"/>
</u:authority>
<html>
<head>
	<title>铃声审核</title>
...
<body>
	<div class="main_search">
        <form method="post" id="mainForm" action="${ctx}/ringAudit/query">
          <ul>
            <li>
           ...
           <li class="time">
            ...
            <li>
            	<u:select id="audit_status" value="${param.audit_status}" placeholder="审核状态" dictionaryType="ring_audit_status" includeValue=",1,2,3" />
            </li>
              <li>
            	<u:select id="upload_status" value="${param.upload_status}" placeholder="上传状态" dictionaryType="ring_upload_status" includeValue=",1,2,3,4" />
            </li>
            <li>
          ...
          <tbody>
            ...
            <s:iterator value="page.list">
	            <tr>
	            	...
	            	<td>${app_sid}</td>
	            	<td>${app_name}</td>
	            	<td><u:ucparams key="${type}" type="ring_type"/></td>
	            	<td><u:ucparams key="${content_type}" type="ring_content_type"/></td>
	            	<td><u:truncate value="${content_type==1?user_file_name:content}" length="20" /></td>
	            	<td>${create_date}</td>
	            	<td>${update_date}</td>
	            	<td id="${id}_audit_name"><u:ucparams key="${audit_status}" type="ring_audit_status"/></td>
	            	<td><u:ucparams key="${upload_status}" type="ring_upload_status"/></td>
	            	<td>
	            		<s:if test="content_type==1">
							<a href="javascript:;" onclick="play_fun('${ctx}/file/view?path=${RING_BASE}/${file_name}');">播放</a>
	            		</s:if>
	            		<s:if test="upload_status==3 && audit_status==1">
	            			<s:if test="menuId_199">
	            		...
	            		<s:if test="audit_status==1 || audit_status==2">
	            			<s:if test="menuId_201">
	            				| <a href="javascript:;" onclick="pass_uplaod('${id}')">上传</a>
	            			</s:if>
	            		</s:if>
					</td>
	            </tr>
...
<div class="float_box audit_box" id="audit_box_pass" style="display:none;">
  ...
  <div class="float_ctn">
    <form method="post" id="audit_form">
      ...
      <input type="hidden" id="pass_ring_type" name="ring_type"/>
...
<div class="float_box audit_box" id="audit_box_unpaas" style="display:none;">
  ...
  <div class="float_ctn">
      ...
      <p class="btn">
	      ...
	      <input type="hidden" id="unpass_ring_type" name="ring_type"/>
...
function pass_uplaod(id){
	$.ajax({
		type: "post",
		url: "${ctx}/ringAudit/uplaod",
		data:{
			'id' : id
		},
		success: function(data){
