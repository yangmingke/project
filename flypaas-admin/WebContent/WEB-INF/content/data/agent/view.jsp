...
<body>
    <h1><a href="${ctx}/agent/query" class="back">返 回</a>代理商管理/代理商资料</h1>
    ...
    <li><label>代理商状态</label><span><u:ucparams key="${data.status}" type="user_status"/></span></li>
    ...
    <li id="showUrl"><label>代理平台登录网址</label>
    <span>
    	${data.agent_url}
    	<a class="update" id="edit" onclick="editUrl()">修改</a>
    </span></li>
    <li id="editUrl" style="display: none;"><label>代理平台登录地址</label>
	    <span>
	    	<input type="text" id="agent_url" name="agent_url" value="${data.agent_url}" maxlength="50" class="txt_227" placeholder="http://"/>
	    	<a class="commit" onclick="commitUrl(this, '${data.sid}')">确认</a>
	    	<a class="back" id="cancel" onclick="editCancel()">取消</a>
	    </span>
    </li>
    </ul>
    <div class="clear"></div>
...
function commitUrl(btn, sid){
	if(confirm("确定要修改代理平台登录地址吗？")){
		$(btn).attr("disabled", true);
		var agent_url = $('#agent_url').val();
		$.ajax({
			type: "post",
			url: "${ctx}/agent/editUrl",
			data:{
				sid:sid,
				agent_url:agent_url
			},
