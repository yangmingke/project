...
<body>
      ...
      <form method="post" id="mainForm" action="${ctx}/developer/query">
      	<ul>
            <li>
		      ...
		      <s:iterator value="page.list">
				 ...
				 <%--      	<span>
				      	...
				      	<span>
					      	...
					      	<%-- <span name="isProxy_1" ${is_proxy==1 && status == 1? "" : "style='display:none;'"}>
						      	<s:if test="menuId_132">
...
//查看
function view(sid){
	location.href="${ctx}/developer/view?sid=" + sid;
}
...
function updateStatus(a, sid, status){
	...
	if(confirm("确定要"+ $(a).text() +"吗？")){
		$.ajax({
			type: "post",
			url: "${ctx}/developer/updateStatus",
			data:{
				sid:sid,
				status:status
			},
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.result=="success"){
					...
					window.location.href="/developer/query";//刷新当前页面.
				}
				alert(data.msg);
			}
		});
